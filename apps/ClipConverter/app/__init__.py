import socket
from .configuration import FORMAT,BUFFER_SIZE,CLIENT,SERVER,ERROR
import json
...
def commandHandler(conn,data):
    while True:
        ...
        if data[0] == "":
            ...
            if(len(data)<2):
                print(f"{ERROR}{Fore.RED}Wrong amount of parameters{Fore.RESET}")
            else:
                if(data[1]=="-s"):
                    if(len(data)!=5):
                        print(f"{ERROR}{Fore.RED}Wrong amount of parameters{Fore.RESET}")
        ...
        print(f"{ERROR}{Fore.RED}Unknown command{Fore.RESET}")
